...
const excludedPatterns=['RELEASE_MANIFEST.json','.git/**','.cache/**','.venv/**','.venv-oai/**','dist/**','node_modules/**','**/__pycache__/**','**/*.pyc'];
const files=[];
async function walk(dir){
  for(const entry of await readdir(dir,{withFileTypes:true})){
    const full=resolve(dir,entry.name);
    const rel=relative(root,full).split(sep).join('/');
    if(exclusions.has(rel)||rel==='.git'||rel.startsWith('.git/')||rel==='.cache'||rel.startsWith('.cache/')||rel==='.venv'||rel.startsWith('.venv/')||rel==='.venv-oai'||rel.startsWith('.venv-oai/')||rel==='dist'||rel.startsWith('dist/')||rel==='node_modules'||rel.startsWith('node_modules/')||rel.includes('/__pycache__/')||rel.endsWith('.pyc'))continue;
